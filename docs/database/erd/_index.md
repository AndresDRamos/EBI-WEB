...
>
> Last synced: 2026-07-07. Reflects V1–V15 (this sync from the applied migration
> file V15 + regenerated Kysely types — 36 tables — not live introspection;
> `flyway_schema_history` in `EBI_dev` reports schema version 15, success).
...
- [auth](auth.md) — usuarios, roles, departamentos, invitaciones y el registro de navegación del portal (`nav_section`, `nav_item`, `role_nav_section`). `plant` se movió a `org` en V15; `user_plant` sigue aquí.
