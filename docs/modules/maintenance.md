...
- Owns the `maint` schema access layer (`src/lib/db/maint.ts`) — the only
  place that queries `maint.*` tables.
...
- Owns `/api/assets/**` and `/api/processes/**`. Reads require any
...
- Owns the `(portal)/maintenance/*` UI: machines list (generic `DataTable`),
  machine detail (Datos / Procesos / Restricciones / Documentos tabs),
  process catalog, printable QR label.
- Does **not** own plants (module `admin`, `auth.plant`) — assets reference
...
- Restrictions are managed through dedicated sub-routes
  (`/api/assets/[id]/restrictions[/...]`), not inside the asset PATCH payload
  (executor's choice per plan step 5).

## Dependency flow

- `(portal)/maintenance/*` pages → `src/lib/db/maint.ts` + `src/lib/db/org.ts`
  (plant options) + `src/lib/auth/rbac.ts` (`isAdmin` → `canManage` prop).
- `/api/assets/**`, `/api/processes/**` → `maint.ts`; document routes also →
  `src/lib/storage/blob.ts` (Azure Blob).
...
- **Enum values are CHECK constraints, not lookup tables.**
  `src/lib/maintenance/enums.ts` mirrors the CHECKs in V5/V6 — change them
...
- **`src/lib/maintenance/enums.ts` must stay a pure module** (no `server-only`,
  no I/O): it is imported by both client components and API validation.
