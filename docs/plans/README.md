...
| 2026-07-06 | [Rename schema produccion → production](production-schema-rename.md) | Approved | English DB naming convention: V12 `ALTER SCHEMA TRANSFER` of the 3 V11 tables + re-issued schema grants + drop `produccion`; `withSchema("production")` in `modules/production/db.ts`; erd/dictionary pages renamed |
