...
import { findDocumentById, softDeleteDocument } from "@/lib/db/maint";
...
/**
 * GET /api/assets/[id]/documents/[docId] — 302 redirect to a short-lived SAS
...
export async function GET(
  ...
  try {
    ...
    if (res) return res;
    console.error("GET /api/assets/[id]/documents/[docId] failed:", err);
    return NextResponse.json(
...
/**
 * DELETE /api/assets/[id]/documents/[docId] — soft delete (admin). The blob
...
export async function DELETE(
  ...
  try {
    ...
    const res = authErrorResponse(err);
    if (res) return res;
    console.error("DELETE /api/assets/[id]/documents/[docId] failed:", err);
    return NextResponse.json(
