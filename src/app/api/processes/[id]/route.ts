...
import { updateProcess, deleteProcess } from "@/lib/db/maint";
...
/** PUT /api/processes/[id] — update a process (admin). */
export async function PUT(
  ...
  try {
    ...
    console.error("PUT /api/processes/[id] failed:", err);
...
/** DELETE /api/processes/[id] — hard delete (admin); 409 if assets link it. */
export async function DELETE(
  ...
  try {
    ...
    const res = authErrorResponse(err);
    if (res) return res;
    console.error("DELETE /api/processes/[id] failed:", err);
    return NextResponse.json(
