...
import { listProcesses, createProcess } from "@/lib/db/maint";
import { requireUser, requireAnyRole } from "@/lib/auth/rbac";
import { authErrorResponse, parseJsonBody } from "@/lib/auth/api";

/** GET /api/processes — list manufacturing processes (any authenticated user). */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error("POST /api/processes failed:", err);
    return NextResponse.json(
