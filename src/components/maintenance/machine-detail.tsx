...
import {
  ...
  type PlantOption,
  type ParentOption,
} from "@/components/maintenance/machine-form-dialog";
import {
  StatusBadge,
  CriticalityBadge,
...
function RestriccionesTab({
  ...
  async function onSubmit() {
    ...
    try {
      ...
      const url = edit
        ? `/api/assets/${assetId}/restrictions/${edit.restriction_id}`
        : `/api/assets/${assetId}/restrictions`;
  ...
  async function onDeactivate(r: RestrictionItem) {
    const res = await fetch(
      `/api/assets/${assetId}/restrictions/${r.restriction_id}`,
      { method: "DELETE" },
    );
    if (res.ok) onChanged();
  }
...
function DocumentosTab({
  ...
  async function onDelete(d: DocumentItem) {
    const res = await fetch(
      `/api/assets/${assetId}/documents/${d.document_id}`,
      { method: "DELETE" },
    );
    if (res.ok) onChanged();
  }
        ...
        <ul className="divide-y">
          {active.map((d) => (
            <li key={d.document_id} className="flex items-center gap-3 py-2.5">
              ...
              <a
                href={`/api/assets/${assetId}/documents/${d.document_id}`}
                target="_blank"
                rel="noreferrer"
